...
p_dots <- ggplot(data_processed_after_exclusions, aes(x = AMP_score, y = mean_evaluation)) +
...
from <- c(xmin = 0.45, xmax = 0.625, ymin = 0.7, ymax = 1.3)
...
```{r}
  ...
  p_raincloud <- ggplot(data_processed_after_exclusions, aes(x = gender, y = mean_evaluation, fill = gender)) +
    # clouds (flat violins)
    geom_violin(trim = FALSE, alpha = 0.2, scale = "width",
                position = position_nudge(x = rain_height + .05)) +
    # rain (points)
    geom_point(aes(color = gender), size = 1, alpha = 0.8, position = position_jitter(width = rain_height)) +
    # boxplots
    geom_boxplot(width = rain_height, alpha = 0.4, outlier.shape = NA,
                 position = position_nudge(x = -rain_height * 5)) +
    # summary statistics (mean and standard error)
    stat_summary(fun.data = mean_se, geom = "errorbar", width = rain_height, 
                 position = position_nudge(x = rain_height * 8), color = "black") +
    stat_summary(fun = mean, geom = "point", 
                 position = position_nudge(x = rain_height * 8), color = "black", size = 2) +
    # adjust layout
...
```
#Same plot but potentially with better readability: 
...
# Define the height for raincloud plot components
rain_height <- .1
jitter_width <- rain_height / 2  # Reduced jitter width for points
errorbar_width <- rain_height / 2  # Reduced error bar width
...
p_raincloud <- ggplot(data_processed_after_exclusions, aes(x = gender, y = mean_evaluation, fill = gender)) +
  ...
  # rain (points)
  geom_point(aes(color = gender), size = 1, alpha = 0.8, position = position_jitter(width = jitter_width)) +
  # boxplots
  geom_boxplot(width = rain_height / 2, alpha = 0.6, outlier.shape = NA,  # Adjusted boxplot width and alpha
               position = position_nudge(x = -rain_height)) +
  # summary statistics (mean and standard error)
  stat_summary(fun.data = mean_se, geom = "errorbar", width = errorbar_width,  # Adjusted error bar width
               position = position_nudge(x = rain_height * 2), color = "black") +
  stat_summary(fun = mean, geom = "point", 
               position = position_nudge(x = rain_height * 2), color = "black", size = 2) +
  # adjust layout
  scale_x_discrete(name = "", expand = expansion(add = c(rain_height, 0))) +
  scale_y_continuous(name = "Mean Self-Reported Evaluation") +
  coord_flip() +
  facet_wrap(~gender, scales = "free_y", ncol = 1, labeller = label_value) +  # Corrected labeller function
  # custom colors and theme
  theme_minimal() +
  theme(legend.position = "none", axis.text.x = element_blank(), axis.ticks.x = element_blank())  # Cleaned up x-axis
...
p_raincloud <- ggplot(data_processed_after_exclusions, aes(x = gender, y = mean_evaluation, fill = gender)) +
  ...
  # adjust layout
  scale_x_discrete(name = "", expand = expansion(add = c(rain_height, 0)),
                   labels = function(x) toupper(x)) +  # Capitalize x-axis labels
  scale_y_continuous(name = "Mean Self-Reported Evaluation") +
  coord_flip() +
  facet_wrap(~gender, scales = "free_y", ncol = 1, labeller = label_value) +  # Corrected labeller function
  # custom colors and theme
...
```{r}
# Assuming p_raincloud, p, and p_dots are your ggplot objects
library(patchwork)
combined_plots <- (p_raincloud | p) / p_dots
...
# Save as PNG
ggsave("combined_plots.png", combined_plots, width = 10, height = 6, dpi = 300)

# Save as PDF
ggsave("combined_plots.pdf", combined_plots, width = 10, height = 6)
